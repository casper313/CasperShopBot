...
function loadRatings() {
  try {
    if (fs.existsSync(RATINGS_FILE)) {
      const data = JSON.parse(
        fs.readFileSync(RATINGS_FILE, 'utf8')
      );
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.isChatInputCommand()) {
    ...
    const customer = await findTicketCustomer(
      interaction.channel
    );
  ...
  if (interaction.isButton()) {
    ...
    if (
      interaction.customId.startsWith('copy_code:')
    ) {
    ...
    if (
      interaction.customId.startsWith('rate_order:')
    ) {
      ...
      const ratingButtons = [1, 2, 3, 4, 5].map(
        rating =>
          new ButtonBuilder()
            .setCustomId(
